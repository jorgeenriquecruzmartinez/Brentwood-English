...
function setup(parts){
  ...
  shell(`<div class="setup-card"><div class="eyebrow">${esc(cfg.label)}</div><h1>Ready to begin?</h1><p>Enter the student's name. ${cfg.needsAudio?'Complete the audio check before starting.':''}</p>
    ...
    ${cfg.needsAudio?`<div class="check-row"><span class="status-dot" id="audioDot"></span><div class="check-copy"><strong>Audio</strong><small id="audioText">Play the sample and confirm you can hear it.</small></div><button class="btn secondary small" id="audioCheck">Play sample</button></div>`:''}
  ...
  if(cfg.needsAudio) $('#audioCheck').onclick=async()=>{
    const b=$('#audioCheck'); b.disabled=true; $('#audioText').textContent='Loading audio…';
    try{ const a=new Audio('/media/audio/hardware-check.mp3'); a.preload='auto'; await a.play(); $('#audioText').textContent='Playing…'; await new Promise((res,rej)=>{a.onended=res;a.onerror=rej}); audioOK=true; $('#audioDot').classList.add('ok'); $('#audioText').textContent='Audio playback completed.'; }
    catch(e){ audioOK=false; $('#audioDot').classList.add('bad'); $('#audioText').textContent='Audio could not play. Check device volume and try again.'; }
...
function renderCloze(task){
  ...
  const html=esc(task.data.text).replace(/([A-Za-z]+)___/g,(m,prefix)=>{const i=idx++;return `${esc(prefix)}<input class="cloze-input" data-i="${i}" aria-label="Missing letters ${i+1}" value="${esc(saved[i]||'')}">`});
  ...
  $$('.cloze-input').forEach(inp=>inp.oninput=()=>{ const arr=RUNTIME.attempt.responses[key]||[]; arr[+inp.dataset.i]=inp.value.trim(); RUNTIME.attempt.responses[key]=arr; saveActive(); });
}
...
function audioPanel(title,sub,audio,playedKey){ const played=!!RUNTIME.attempt.responses[playedKey+'-played']; return `<div class="audio-task"><div class="audio-card"><div class="audio-visual">◉</div><h2>${esc(title)}</h2><p>${esc(sub)}</p><audio class="audio-player" id="taskAudio" preload="auto" src="${esc(audio)}"></audio><button class="btn play-once" id="playAudio" ${played?'disabled':''}>${played?'Audio played':'Play audio'}</button><div class="audio-status" id="audioStatus">${played?'Audio can only be played once in this attempt.':'Tap Play audio. If playback fails, a Retry button will appear.'}</div></div></div>`; }
function attachAudioOnce(audio,playedKey,onEnded){ const btn=$('#playAudio'), a=$('#taskAudio'), status=$('#audioStatus'); if(!btn||!a)return; btn.onclick=async()=>{btn.disabled=true;status.textContent='Loading…';try{await a.play();status.textContent='Playing…';RUNTIME.currentAudio=a;a.onended=()=>{RUNTIME.attempt.responses[playedKey+'-played']=true;saveActive();status.textContent='Audio completed.';onEnded?.();};a.onerror=()=>{btn.disabled=false;status.textContent='Audio could not load. Tap again to retry.'}}catch(e){btn.disabled=false;status.textContent='Playback was blocked or failed. Tap again to retry.'}}; }
